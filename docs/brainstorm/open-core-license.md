...
**Why this fits Thai Chili Peppers's situation:**
...
- **Trademark — separate the code license from the name.** Open-sourcing the code does **not** open-source the **"Thai Chili Peppers"** name or logo. State explicitly that the trademark is **not** licensed under the AGPL, that forks must be **renamed/rebranded** and may not imply endorsement, and reserve "Thai Chili Peppers" for official builds. **Register the Thai Chili Peppers word mark** (and logo) in Thailand, and in any other target market, before a public launch. Apache-2.0 already disclaims trademark grants; with AGPL, add an explicit `TRADEMARK.md` / `NOTICE` to make the same point.
